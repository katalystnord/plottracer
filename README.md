...
- **Bars are captured as a drag-box, corner to corner.** A bar's value is its *extent*, not a point on it, so the two dragged corners **are** the measurement — nothing is averaged or centroided away. That covers plain, grouped, stacked and floating bars, and a bar below the baseline reads negative. **Auto-extract by colour works on Bar and Histogram** for exactly the same reason: a bar blob's own *bounding box* is its two ends. Box Plot and categorical-X Line still decline it — a box's five letter-values and a categorical point are not a bounding box — rather than quietly return the midpoint.
