...
**Chart / axes types** - XY, Bar, Polar, **Spider / Radar** (every spoke calibrated separately, so each axis keeps its own range), **Pie / Donut** (calibrated from the *outline*, so the centre is fitted rather than clicked - which is what makes a donut, an **exploded slice** and a **tilted 3D** pie readable), **Heatmap** (x and y each independently a *category* or a *value*, with the colour key calibrated as a third axis), Ternary, Map (scale bar), Circular Chart Recorder, Histogram (captures true bin *edges*, not just centres), Box Plot, **Span chart** (a bar that floats clear of the axis - a temperature range, a tornado chart - reporting both measured ends, and carrying its own error bars on each end), **Candlestick** (four marks per candle, walked bottom-up by position, with each period's direction *measured* from the body's own colour and a single tick to flip a figure drawn the other way round), Line with a categorical X axis. Error bars are **not** a graph type - they are a rail tool, captured on top of whichever series they belong to.
...
- **Bars are captured as a drag-box, corner to corner.** A bar's value is its *extent*, not a point on it, so the two dragged corners **are** the measurement - nothing is averaged or centroided away. That covers plain, grouped and stacked bars, and a bar below the baseline reads negative. A bar that floats clear of the axis is a **Span chart**, a type of its own since v2.5 - so Bar means exactly what its name says, measured from the figure's own origin. **Auto-extract by colour works on Bar and Histogram** for exactly the same reason: a bar blob's own *bounding box* is its two ends. Box Plot and Line still decline it - a box's five letter-values and a categorical point are not a bounding box - rather than quietly return the midpoint.
