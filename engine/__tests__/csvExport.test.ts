...
describe('buildTupleDataCSV', () => {
  it('writes one row per tuple with one column per slot', () => {
    ...
    const rows: TupleRow[] = [
      {
        tupleIndex: 0,
        position: null, positionIsBand: false, positionSpan: null,
  ...
  it('exports a blank cell for a still-open slot', () => {
    ...
    const rows: TupleRow[] = [{ tupleIndex: 0, position: null, positionIsBand: false, positionSpan: null, label: 'Bar0', derived: null, points: [{ px: 0, py: 0, data: [1] }, null] }];
  ...
  it('quotes a category label containing a comma', () => {
    ...
    const rows: TupleRow[] = [{ tupleIndex: 0, position: null, positionIsBand: false, positionSpan: null, label: 'Sample, batch 2', derived: null, points: [{ px: 0, py: 0, data: [1] }] }];
  ...
  describe('the derived-value column (v2.0)', () => {
    it('adds no column when no derivedLabel is given -- unchanged from before this fix', () => {
      ...
      const rows: TupleRow[] = [
        { tupleIndex: 0, position: null, positionIsBand: false, positionSpan: null, label: 'Slice A', derived: 42, points: [{ px: 0, py: 0, data: [10] }, { px: 0, py: 0, data: [20] }] },
      ];
    ...
    it('leaves the derived cell blank for a tuple with no derived value, not a fabricated 0', () => {
      ...
      const rows: TupleRow[] = [
        { tupleIndex: 0, position: null, positionIsBand: false, positionSpan: null, label: 'Slice A', derived: 42, points: [{ px: 0, py: 0, data: [10] }, { px: 0, py: 0, data: [20] }] },
        { tupleIndex: 1, position: null, positionIsBand: false, positionSpan: null, label: 'Slice B', derived: null, points: [{ px: 0, py: 0, data: [20] }, null] },
      ];
      expect(buildTupleDataCSV(groupNames, rows, FULL_PRECISION_ROUNDER, ',', 'Value')).toBe(
        'category,Sector start,Sector end,Value\nSlice A,10,20,42\nSlice B,20,,'
      );
    ...
    it('omits the column entirely when a label is declared but every row is null (matches hasRoles-style presence rule)', () => {
      const groupNames = ['Min', 'Q1'];
      const rows: TupleRow[] = [{ tupleIndex: 0, position: null, positionIsBand: false, positionSpan: null, label: 'Sample A', derived: null, points: [{ px: 0, py: 0, data: [1] }, null] }];
